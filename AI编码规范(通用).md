...
### 9.4 可验证交付
* AI **必须**将任务转化为可验证的具体结果，**必须**通过实际结果判断任务完成度，**严禁**仅凭主观逻辑认定任务完成。
* 交付的代码**必须**包含明确的验证方式或配套的测试用例。
...
- **协作类**：盲猜需求、跳过计划审查直接编码（9.1）、不经多方案对比锁定唯一实现（9.1）、随便验证了事即交付（9.4）、新项目未询问用户自行假定命名规范（2）
