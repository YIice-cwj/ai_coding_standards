...
### 3.2 函数注释
- **所有函数必须包含注释**
- 注释**必须**使用 `/** */` 文档注释格式，**禁止**使用 `//` 行注释代替函数注释
...
- **禁止**无注释或注释不完整
  ...
  **反例**：
  ...
  // 计算用户得分          // 错！禁止用 // 代替函数注释
  int calc_score(const user_t& user);
  int calc_score(const user_t& user);   // 错！无注释
  ```
...
- 版本号升级**必须**遵循语义化版本（SemVer）：
  - **主版本（MAJOR）**：不兼容变更，如删除/重写既有章节、反转规则语义（如某条"必须"改为"禁止"）
  - **次版本（MINOR）**：新增章节、新增规则、补充正反例、新增附录
  - **修订号（PATCH）**：修正笔误、调整格式、优化措辞、修正链接
- 提交信息**必须**在变更描述中明确写出"版本 X.Y.Z → X.Y.Z"的升级轨迹（与第23条 Git 规范衔接）
