...
**版本**: 1.0.0  **日期**: 2026-08-09

> **适用范围**：所有 UE5 项目 C++ 新代码；修改老代码时遵循最小变更原则（见 26.3），不强制重构未触及的代码
> **基准规范**：本规范派生自《C++ AI 编码规范 v4.9.0》，在 UE5 引擎约束下做适配与仲裁；两者冲突时以本规范为准
...
### 11.3 非反射类的模板
非反射的纯 C++ 工具类**可以**使用模板，按 C++ 规范 11 实现。
...
> **继承 C++ 规范 14**，UE5 默认禁用 C++ 异常，与本规范天然契合：
...
> **覆盖 C++ 规范 15**：UE5 项目**禁止**使用 `cout` / `printf` / `std::format`，**必须**使用 `UE_LOG`。
...
> **覆盖 C++ 规范 16.1**：UE5 项目优先使用 UE5 同步原语。
...
> **覆盖 C++ 规范 16.2**：UE5 项目**禁止**使用 `std::thread` / `std::jthread`，**必须**使用 UE5 任务系统。
...
> **覆盖 C++ 规范 19.1 中的 `std::string_view`**：
...
> **覆盖 C++ 规范 20**：UE5 项目使用 Automation 测试框架。
...
- **必须**在已知元素数量时调用 `Reserve()` 预分配
- **必须**优先使用 `EmplaceBack` / `Emplace` 替代 `Add`，避免临时对象
- **禁止**对顺序容器使用 `std::list`（缓存不友好）
